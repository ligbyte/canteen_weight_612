...
public final class TimeUtils {
     ...
     * @return {@code true}: yes<br>{@code false}: no
     *//**
 * Return the current date formatted as "yyyy年M月d日 E".
 *
 * @return the current date formatted as "yyyy年M月d日 E"
 */
public static String getCurrentDateWithChineseDay() {
    return new SimpleDateFormat("yyyy年M月d日 E", Locale.CHINA).format(new Date());
}
     ...
     * @return the current date in the format yyyy-MM-dd
     */
    public static String getLastDateFormatted() {
        Calendar calendar = Calendar.getInstance();
        int year = calendar.get(Calendar.YEAR);
     ...
     * 计算两个日期之间的天数差
     *
     * @param startTime 开始日期，格式为 yyyyMMdd
     * @param endTime   结束日期，格式为 yyyyMMdd
     * @return 两个日期之间的天数差
     */
    public static long getDaysBetween(String startTime, String endTime) {
        ...
        try {
            ...
            // 计算天数差（忽略时分秒，只比较日期）
            long daysDifference = (endMillis - startMillis) / TimeConstants.DAY;

            return daysDifference;
        } catch (ParseException e) {
            e.printStackTrace();
            // 可以根据需求处理异常，例如抛出自定义异常或返回-1
            return -1;
        }
    }
