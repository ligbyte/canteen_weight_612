...
import com.stkj.plate.weight.base.callback.OnConsumerConfirmListener;
...
import com.stkj.plate.weight.base.ui.dialog.CashierPayDialogFragment;
...
import com.stkj.plate.weight.pay.callback.OnPayListener;
...
import com.stkj.plate.weight.pay.helper.PayHelper;
import com.stkj.plate.weight.pay.model.CategoryBean;
import com.stkj.plate.weight.pay.model.ChangeConsumerModeEvent;
import com.stkj.plate.weight.pay.model.ConsumeFoodInfo;
import com.stkj.plate.weight.pay.model.FacePassRetryEvent;
import com.stkj.plate.weight.pay.model.FoodCategoryListInfo;
import com.stkj.plate.weight.pay.model.GoodsClearAllEvent;
import com.stkj.plate.weight.pay.model.GoodsInventoryListInfoViewHolder;
import com.stkj.plate.weight.pay.model.ModifyBalanceResult;
import com.stkj.plate.weight.pay.model.RefreshConsumerGoodsEvent;
import com.stkj.plate.weight.pay.model.RefreshConsumerGoodsModeEvent;
import com.stkj.plate.weight.pay.model.RefreshGoodsModeEvent;
import com.stkj.plate.weight.pay.model.RefreshUpdateGoodsEvent;
import com.stkj.plate.weight.pay.model.TabEntity;
import com.stkj.plate.weight.pay.model.UpdateBlanceEvent;
import com.stkj.plate.weight.pay.service.PayService;
import com.stkj.plate.weight.pay.ui.adapter.FoodGridAdapter;
import com.stkj.plate.weight.pay.ui.weight.GoodsAutoSearchLayout;
...
import com.stkj.plate.weight.setting.data.TTSSettingMMKV;
import com.stkj.plate.weight.setting.model.FacePassPeopleInfo;
            ...
            public void afterTextChanged(Editable s) {
                ...
                if (text.isEmpty()) {
                    ...
                    if (MainApplication.hasWeight) {
                        MainApplication.hasWeight = false;
                        foodGridAdapter.notifyDataSetChanged();
                    }
                }else {
                    if (!MainApplication.hasWeight){
                        MainApplication.hasWeight = true;
                        foodGridAdapter.notifyDataSetChanged();
                    }
                }
            }
                ...
                .setOnSelectListener(new CashierPayDialogFragment.OnSelectListener() {
                    ...
                    public void onConfirmSelectItem(int currentMode, String payMoney) {
                        if (currentMode == CashierPayDialogFragment.MODE_QUICK){
                            ...
                            cashierPayDialogFragment.dismiss();
                            MainApplication.consumeFoodInfoList  = new ArrayList<>();
                            for (int i = 0; i < goodsInventoryAdapter.getDataList().size(); i++) {
                                FoodInfoTable foodInfoTable = ((FoodInfoTable) (goodsInventoryAdapter.getDataList().get(i)));
                            ...
                            MainApplication.isJuHePay = false;
        ...
        if (v.getId() == R.id.pay_money) {
            ...
            if (goodsInventoryAdapter.getDataList().size() > 0) {
                if (stv_qrcode.getText().equals("取消结算")){
                    ...
                    MainApplication.isJuHePay = true;
                    goToAmountPay();
                }
