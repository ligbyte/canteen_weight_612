...
import com.stkj.plate.weight.pay.callback.OnCalculateListener;
 ...
 * 收银计算器
 */
public class CashierCalculator extends FrameLayout {
    ...
    public CashierCalculator(@NonNull Context context) {
        super(context);
        init(context, null);
    }

    public CashierCalculator(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public CashierCalculator(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attributeSet) {
        ...
        if (attributeSet != null) {
            TypedArray array = context.obtainStyledAttributes(attributeSet, R.styleable.SimpleCalculator);
            confirmTxt = array.getString(R.styleable.SimpleCalculator_sc_confirm_txt);
        }
        LayoutInflater.from(context).inflate(R.layout.include_fast_pay_calculator_cashier, this);
        tvConsume = (EditText) findViewById(R.id.tv_consume);
        ...
        findViewById(R.id.stv_cancel).setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                if (onCalculateListener != null) {
                    onCalculateListener.onCancel();
                }
            }
        });

        tvConsume.setOnEditorActionListener(new TextView.OnEditorActionListener() {
            @Override
...
//        });
    ...
    private void confirmOperateNumberResult() {
        ...
        if (TextUtils.isEmpty(currentInputText)) {
            if (onCalculateListener != null) {
                onCalculateListener.onConfirmMoney("");
            }
            return;
        }
        //分割最后的输入的数据
